...
public class Manuscript
{
   ...
   /**Used internally when large numbers of manuscripts are being instantiated via a single query*/
    ...
    /**Create and store a new manuscript. Generates a new manuscript id.*/
    public Manuscript(String repository, String archive, String collection, String city, int restricted) throws SQLException
    {
        String query="INSERT INTO manuscript (repository, archive, msIdentifier, city, restricted) "
                + "SELECT ?,?,?,?,? "
                + "FROM dual "
                + "WHERE NOT EXISTS ("
                + "SELECT * FROM manuscript "
        ...
        try {
            ...
            ps.setString(3, collection);
            ps.setString(4, city);
            ps.setInt(5, restricted);
            ps.setString(6, repository);
