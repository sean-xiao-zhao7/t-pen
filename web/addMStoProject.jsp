        ...
        <%        } else {
            ...
            if (request.getParameter("newProject") != null) {
                ...
                try (Connection conn = ServletUtils.getDBConnection()) {
                   ...
                   FolioSet f = new FolioSet(conn, projectFolio.getArchive(), projectFolio.getCollectionName());
                   Manuscript ms = new Manuscript(folioNum);
                   newProject.setFolios(conn, ms.getFolios());

                   newProject.addLogEntry(conn, "<span class='log_manuscript'></span>Added manuscript " + ms.getShelfMark(), UID);
...
%>
        ...
        <%
            ...
            if (request.getParameter("projectID") != null) {
                ...
                try (Connection conn = ServletUtils.getDBConnection()) {
                   conn.setAutoCommit(false);
                   newProject.setFolios(conn, allFolios);
                   Manuscript ms = new Manuscript(folioNum);
                   newProject.addLogEntry(conn, "<span class='log_manuscript'></span>Added manuscript " + ms.getShelfMark(), UID);
                   conn.commit();
